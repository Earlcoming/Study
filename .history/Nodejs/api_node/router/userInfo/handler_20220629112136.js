...
exports.updatePassword = (req, res) => {
  ...
  db.query(selectPass, req.auth.id, (err, results) => {
    ...
    const oldPass = bcrypt.hashSync(req.body.oldPass, 10);
    const comparePass = bcrypt.compareSync(oldPass, results[0].password);
    console.log(oldPass, results[0].password);
    if (comparePass) return res.msg(msg.userUpdatePas);
  });
